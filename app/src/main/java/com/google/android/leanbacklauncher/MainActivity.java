...
import com.google.android.leanbacklauncher.util.TvSearchIconLoader;
import com.google.android.leanbacklauncher.util.TvSearchSuggestionsLoader;
...
public class MainActivity extends Activity implements OnEditModeChangedListener, OnEditModeUninstallPressedListener {
                    ...
                    switch (tag) {
                        case 0:
                            if (child instanceof SearchOrbView) {
                                ((SearchOrbView) child).setLaunchListener(new SearchLaunchListener() {
    ...
    private boolean checkFirstRunAfterBoot() {
        ...
        if (firstRun) {
            ((AlarmManager) getSystemService("alarm")).set(2, SystemClock.elapsedRealtime() + 864000000000L, PendingIntent.getActivity(this, 0, dummyIntent, 0));
        }
        return firstRun;
    }
