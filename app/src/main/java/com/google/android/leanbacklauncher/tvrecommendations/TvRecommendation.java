...
public final class TvRecommendation implements Parcelable {
    public static final Creator<TvRecommendation> CREATOR;
    ...
    /* renamed from: com.google.android.tvrecommendations.TvRecommendation.1 */
    static class C02271 implements Creator<TvRecommendation> {
        C02271() {
        }
    ...
    TvRecommendation(Parcel in) {
        ...
        this.mContentIntent = (PendingIntent) in.readParcelable(classLoader);
        if (in.readInt() != 0) {
            z = true;
        } else {
            z = false;
        ...
        this.mContentImage = (Bitmap) in.readParcelable(classLoader);
        ...
        this.mSourceName = (CharSequence) TextUtils.CHAR_SEQUENCE_CREATOR.createFromParcel(in);
