...
public class ActiveFrame extends LinearLayout implements HomeScrollFractionListener, ParticipatesInScrollAnimation {
    ...
    protected void onFinishInflate() {
        ...
        if (this.mHeader != null) {
            ...
            this.mHeaderFadeOutAnimation = new FadeAnimator(this.mHeader, Direction.FADE_OUT);
        }
    }
    ...
    public void onScrollPositionChanged(int position, float fractionFromTop) {
        if (this.mHeader != null) {
            ...
            int height = (int) (((float) this.mHeaderHeight) * fractionFromTop);
            if (lp.height != height) {
                lp.height = height;
                this.mHeader.setLayoutParams(lp);
            }
        }
        setPadding(0, 0, 0, Math.round((((float) (this.mBottomPadding - this.mRowMinSpacing)) * fractionFromTop) + ((float) this.mRowMinSpacing)));
    }
