...
import com.google.android.leanbacklauncher.settings.RecommendationsPreferenceManager.LoadRecommendationPackagesCallback;
import com.google.android.leanbacklauncher.settings.RecommendationsPreferenceManager.PackageInfo;
import java.util.ArrayList;
import java.util.HashMap;
import java.util.List;

public class LegacyRecommendationsPreferenceFragment extends GuidedStepFragment implements LoadRecommendationPackagesCallback {
    ...
    public void onRecommendationPackagesLoaded(List<PackageInfo> infos) {
        if (isAdded()) {
            ...
            for (PackageInfo packageInfo : infos) {
                ...
                actions.add(((Builder) ((Builder) ((Builder) ((Builder) ((Builder) new Builder(getActivity()).id(actionId)).title(packageInfo.appTitle)).icon(banner)).checkSetId(-1)).checked(!packageInfo.blacklisted)).build());
                this.mActionToPackageMap.put(Long.valueOf(actionId), packageInfo.packageName);
                actionId++;
            }
            setActions(actions);
        }
