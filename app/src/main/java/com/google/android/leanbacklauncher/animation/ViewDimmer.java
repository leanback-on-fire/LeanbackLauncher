...
public class ViewDimmer {
    ...
    public void setDimLevel(float level) {
        ...
        if (this.mPlayingIndicatorViews != null) {
            ...
            for (i = 0; i < size; i++) {
                ((PlayingIndicatorView) this.mPlayingIndicatorViews.get(i)).setColorFilter(filter);
            }
        }
        if (this.mDesatDrawables != null) {
            size = this.mDesatDrawables.size();
            for (i = 0; i < size; i++) {
                ((Drawable) this.mDesatDrawables.get(i)).mutate();
                ((Drawable) this.mDesatDrawables.get(i)).setColorFilter(desatFilter);
            }
        }
    }
