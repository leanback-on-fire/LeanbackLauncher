  ...
  implements AppsManager.AppsViewChangeListener
{
  ...
  FavoriteItemsManager(Context paramContext)
  {
    ...
    this.mMoreFavoritesItem = new LaunchItem(this.mContext, this.mContext.getString(2131492991), "com.google.android.tvlauncher.appsview.FavoriteItemsManager.MORE_FAVORITES_PKGNAME", new Intent(this.mContext, FavoriteAppsActivity.class), this.mContext.getDrawable(2130837631));
    ...
    {
      paramContext = Partner.get(this.mContext).getOutOfBoxFavoriteAppsList();
      if (paramContext != null)
      {
        int i = 0;
        while ((i < paramContext.length) && (i < 6))
        {
          this.mDefaultItemsOrder.put(paramContext[i], Integer.valueOf(i));
          i += 1;
        }
  ...
  private List<LaunchItem> getSortedFavorites()
  {
    ...
    Collections.sort(localArrayList, new LaunchItemUserOrderComparator(null));
  ...
  {
    paramLaunchItem = (Integer)this.mFavoriteItems.remove(paramLaunchItem);
    ...
    return paramLaunchItem.intValue();
  }
  ...
  public void onLaunchItemsRemoved(ArrayList<LaunchItem> paramArrayList)
  {
    ...
    {
      if (removeFavorite((LaunchItem)paramArrayList.next()) == -1) {}
      for (int j = 1;; j = 0)
      {
        i |= j;
        break;
  ...
  public void swapAppOrder(LaunchItem paramLaunchItem1, LaunchItem paramLaunchItem2)
  {
    ...
    int i = ((Integer)this.mFavoriteItems.get(paramLaunchItem1)).intValue();
    int j = ((Integer)this.mFavoriteItems.get(paramLaunchItem2)).intValue();
  ...
  void userAddToFavorites(LaunchItem paramLaunchItem)
  {
    if ((!this.mFavoriteItems.containsKey(paramLaunchItem)) && (this.mFavoriteItems.size() < 6))
    {
      ...
      paramLaunchItem = getSortedFavorites();
      saveOrderSnapshot(paramLaunchItem);
      notifyOnHomeScreenItemsChanged(paramLaunchItem);
    }
  }
  ...
  private class LaunchItemUserOrderComparator
    implements Comparator<LaunchItem>
  {
    ...
    public int compare(LaunchItem paramLaunchItem1, LaunchItem paramLaunchItem2)
    {
      ...
      Integer localInteger2 = (Integer)FavoriteItemsManager.this.mFavoriteItems.get(paramLaunchItem1);
      Integer localInteger1 = (Integer)FavoriteItemsManager.this.mFavoriteItems.get(paramLaunchItem2);
      if (localInteger2 == null)
      {
        i = FavoriteItemsManager.this.mFavoriteItems.keySet().size();
      ...
      label128:
      for (int i = FavoriteItemsManager.this.mFavoriteItems.keySet().size();; i = localInteger1.intValue())
      {
...
/* Location:              /home/evan/Downloads/fugu-opr2.170623.027-factory-d4be396e/fugu-opr2.170623.027/image-fugu-opr2.170623.027/TVLauncher/TVLauncher/TVLauncher-dex2jar.jar!/com/google/android/tvlauncher/appsview/FavoriteItemsManager.class
