...
public class FireTVUtils {
    ...
    public static boolean isLocalNotificationsEnabled(Context context) {
        try {
            PackageManager packageManager = context.getPackageManager();
            return packageManager.getApplicationInfo("com.amazon.device.sale.service", 0).enabled;
        } catch (NameNotFoundException unused) {
            return false;
        }
    }

    public static boolean isLauncherSettingsEnabled(Context context) {
        try {
            PackageManager packageManager = context.getPackageManager();
